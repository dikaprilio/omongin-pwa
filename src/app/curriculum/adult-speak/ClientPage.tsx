...
import { speakingAdultPhases, getIdsByCurriculum } from '../data';
...
export default function AdultSpeakPage() {
    const availableMeetingIds = getIdsByCurriculum('speaking-adults');

    // Flatten all meetings into a single list
    const allMeetings = speakingAdultPhases.flatMap(phase => phase.meetings);
            ...
            <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-gray-400">30 Topics</span>
                    </div>
                </div>
            </header>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
                ...
                <div className="text-center max-w-2xl mx-auto mb-12">
                    <h1 className="text-4xl sm:text-5xl font-black text-gray-900 tracking-tight mb-4 transform -rotate-1">
                        Speaking <span className="text-blue-600 relative inline-block">
                            Mastery
                    ...
                    <p className="text-lg text-gray-600 font-medium leading-relaxed">
                        Dari <span className="bg-red-100 text-red-700 font-bold px-1 transform -rotate-2 inline-block mx-1">Grogi</span>
                        jadi <span className="bg-green-100 text-green-700 font-bold px-1 transform rotate-1 inline-block mx-1">Percaya Diri</span>.
                        Selesaikan semua misinya!
                    </p>
                </div>
                        ...
                        return (
                            <motion.div
                                ...
                                className={`group relative bg-white rounded-3xl border transition-all duration-300 overflow-hidden ${isAvailable
                                        ? 'border-gray-200 hover:border-blue-300 hover:shadow-xl hover:shadow-blue-100'
                                ...
                                <div className="p-6">
                                    <div className="flex items-start justify-between mb-4">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div
                                                className={`w-12 h-12 flex items-center justify-center text-lg font-bold shadow-sm transition-transform group-hover:rotate-12 ${isAvailable ? 'bg-yellow-400 text-gray-900' : 'bg-gray-100 text-gray-400'
                                                    }`}
                                    ...
                                    {isAvailable ? (
                                        <div className="grid grid-cols-2 gap-3">
                                            <Link
                                                href={`/curriculum/adult-speak/${meeting.id}`}
                                                className="flex items-center justify-center gap-2 py-3 px-4 bg-blue-600 text-white rounded-xl font-bold text-sm hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-200 transition-all"
                                            >
                                                <PlayCircle className="w-4 h-4" />
                                                Materi
                                            </Link>
                                            <Link
                                                href={`/curriculum/adult-speak/quiz/${meeting.id}`}
                                        ...
                                        <div className="w-full py-3 bg-gray-50 rounded-xl text-center text-sm font-bold text-gray-400 border border-gray-100 dashed">
                                            Locked
                                        </div>
                                    )}
                                </div>
