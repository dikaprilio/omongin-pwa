...
import { basicKidsPhases, getIdsByCurriculum } from '../data';
...
export default function BasicKidsPage() {
    const availableMeetingIds = getIdsByCurriculum('basic-kids');

    // Flatten all meetings into a single list
    const allMeetings = basicKidsPhases.flatMap(phase => phase.meetings);
            ...
            <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="px-3 py-1 bg-teal-100 text-teal-800 rounded-full text-xs font-bold uppercase tracking-wider">
                            Basic English
                        </div>
                        <span className="text-sm font-bold text-gray-400">Kids</span>
                    </div>
                </div>
            </header>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
                ...
                <div className="text-center max-w-2xl mx-auto mb-12">
                    <h1 className="text-4xl sm:text-5xl font-black text-gray-900 tracking-tight mb-4 transform rotate-1">
                        Grammar <span className="text-teal-600 relative inline-block">
                            Fun
                    ...
                    <p className="text-lg text-gray-600 font-medium leading-relaxed">
                        Dari <span className="bg-orange-100 text-orange-700 font-bold px-1 transform -rotate-2 inline-block mx-1">Bingung</span>
                        jadi <span className="bg-teal-100 text-teal-700 font-bold px-1 transform rotate-1 inline-block mx-1">Asyik & Paham</span>.
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {allMeetings.map((meeting, idx) => {
                        const isAvailable = true; // Assuming available for now
                        ...
                        return (
                            <motion.div
                                ...
                                className={`group relative bg-white rounded-3xl border transition-all duration-300 overflow-hidden ${isAvailable
                                    ? 'border-gray-200 hover:border-teal-300 hover:shadow-xl hover:shadow-teal-100'
                                ...
                                <div className="p-6">
                                    <div className="flex items-start justify-between mb-4">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div
                                                className={`w-12 h-12 flex items-center justify-center text-lg font-bold shadow-sm transition-transform group-hover:rotate-12 ${isAvailable ? 'bg-teal-400 text-white' : 'bg-gray-100 text-gray-400'
                                                    }`}
                                    ...
                                    <div className="mb-6 bg-gray-50 rounded-2xl p-3 border border-gray-100">
                                        <div className="flex items-start gap-2 text-sm">
                                            ...
                                            <div>
                                                <span className="font-bold text-gray-700 block mb-0.5">Focus:</span>
                                    ...
                                    {isAvailable ? (
                                        <div className="grid grid-cols-2 gap-3">
                                            <Link
                                                href={`/curriculum/basic-kids/${idx + 1}`}
                                                className="flex items-center justify-center gap-2 py-3 px-4 bg-teal-600 text-white rounded-xl font-bold text-sm hover:bg-teal-700 hover:shadow-lg hover:shadow-teal-200 transition-all"
                                            >
                                                <PlayCircle className="w-4 h-4" />
                                                Materi
                                            </Link>
                                            <Link
                                                href={`/curriculum/basic-kids/quiz/${idx + 1}`}
