...
export default function BasicKidsDetailPage() {
    const params = useParams();
    const router = useRouter();
    const localId = Number(params.id);
    const globalId = localId + 120;
        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="text-gray-600 bg-red-50 px-6 py-4 rounded-xl border border-red-100 flex flex-col items-center gap-4">
                    ...
                    <Link href="/curriculum/basic-kids" className="text-sm font-medium text-red-600 hover:underline">
                        Back to Curriculum
                    </Link>
                </div>
            </div>
    ...
    // Basic Kids meetings have local IDs 1-30 mapping to global 121-150
            ...
            {activeTab !== 'quiz' && (
                <div className="bg-white border-t border-gray-200 py-4 px-6 shrink-0 z-30">
                    <div className="max-w-7xl mx-auto flex items-center justify-between">
                        {prevId ? (
                            <Link href={`/curriculum/basic-kids/${prevId}`} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors">
                        ...
                        {nextId ? (
                            <Link href={`/curriculum/basic-kids/${nextId}`} className="flex items-center gap-2 text-sm font-medium text-teal-600 hover:text-teal-700 transition-colors">
                                Next Topic
