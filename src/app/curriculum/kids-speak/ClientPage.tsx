...
import { speakingKidsPhases, getIdsByCurriculum } from '../data';
...
export default function KidsSpeakPage() {
    const availableMeetingIds = getIdsByCurriculum('speaking-kids');

    // Flatten all meetings into a single list
    const allMeetings = speakingKidsPhases.flatMap(phase => phase.meetings);
            ...
            <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="px-3 py-1 bg-pink-100 text-pink-800 rounded-full text-xs font-bold uppercase tracking-wider">
                            Kids Speaking
                        </div>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
                ...
                <div className="text-center max-w-2xl mx-auto mb-12">
                    <h1 className="text-4xl sm:text-5xl font-black text-gray-900 tracking-tight mb-4 transform rotate-1">
                        Speaking <span className="text-pink-600 relative inline-block">
                            Buddy
                    ...
                    <p className="text-lg text-gray-600 font-medium leading-relaxed">
                        Dari <span className="bg-orange-100 text-orange-700 font-bold px-1 transform -rotate-2 inline-block mx-1">Malu-malu</span>
                        jadi <span className="bg-yellow-100 text-yellow-700 font-bold px-1 transform rotate-1 inline-block mx-1">Jago Ngomong</span>.
                        Ayo main dan belajar!
                    </p>
                </div>
                        ...
                        return (
                            <motion.div
                                ...
                                className={`group relative bg-white rounded-3xl border transition-all duration-300 overflow-hidden ${isAvailable
                                    ? 'border-gray-200 hover:border-pink-300 hover:shadow-xl hover:shadow-pink-100'
                                ...
                                <div className="p-6">
                                    <div className="flex items-start justify-between mb-4">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div
                                                className={`w-12 h-12 flex items-center justify-center text-lg font-bold shadow-sm transition-transform group-hover:rotate-12 ${isAvailable ? 'bg-pink-400 text-white' : 'bg-gray-100 text-gray-400'
                                                    }`}
                                    ...
                                    {isAvailable ? (
                                        <div className="grid grid-cols-2 gap-3">
                                            <Link
                                                href={`/curriculum/kids-speak/${idx + 1}`}
                                                className="flex items-center justify-center gap-2 py-3 px-4 bg-pink-600 text-white rounded-xl font-bold text-sm hover:bg-pink-700 hover:shadow-lg hover:shadow-pink-200 transition-all"
                                            >
                                                <PlayCircle className="w-4 h-4" />
                                                Materi
                                            </Link>
                                            <Link
                                                href={`/curriculum/kids-speak/quiz/${idx + 1}`}
