...
export default function KidsTopicPage() {
    const params = useParams();
    const router = useRouter();
    const localId = Number(params.topicId);
    const globalId = localId + 30;
        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="text-gray-600 bg-red-50 px-6 py-4 rounded-xl border border-red-100 flex flex-col items-center gap-4">
                    ...
                    <Link href="/curriculum/kids-speak" className="text-sm font-medium text-red-600 hover:underline">
                        Back to Curriculum
                    </Link>
                </div>
            </div>
    ...
    // Kids meetings have local IDs 1-30 mapping to global 31-60
                ...
                <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
                    ...
                    <div className="flex bg-gray-100 rounded-lg p-1">
                        <button
                            ...
                            className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${activeTab === 'presentation'
                        ...
                        <button
                            ...
                            className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${!meeting?.quiz?.length
            ...
            {activeTab !== 'quiz' && (
                <div className="bg-white border-t border-gray-200 py-4 px-6 shrink-0 z-30">
                    <div className="max-w-7xl mx-auto flex items-center justify-between">
                        {prevId ? (
                            <Link href={`/curriculum/kids-speak/${prevId}`} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors">
                        ...
                        {nextId ? (
                            <Link href={`/curriculum/kids-speak/${nextId}`} className="flex items-center gap-2 text-sm font-medium text-pink-600 hover:text-pink-700 transition-colors">
                                Next Topic
