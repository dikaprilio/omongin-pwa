...
import { getQuizByGlobalId, type TopicQuiz } from '../../../data/bank-soal';
...
export default function QuizClientPage({ topicId }: QuizClientPageProps) {
    ...
    const meetingInfo = useMemo(() => findMeetingById(globalId), [globalId]);

    useEffect(() => {
        async function loadQuiz() {
            setIsLoading(true);
            const data = await getQuizByGlobalId(globalId);
            if (data) {
                setQuiz(data);
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-slate-50 to-white">
                <div className="text-center bg-white rounded-2xl p-8 shadow-lg border border-gray-100 max-w-sm mx-4">
                    ...
                    <h2 className="text-lg font-bold text-gray-900 mb-2">Quiz Belum Tersedia</h2>
                    <p className="text-gray-500 text-sm mb-6">Materi quiz untuk topik ini sedang disiapkan.</p>
                    <Link
        ...
        return (
            <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
                ...
                <main className="max-w-lg mx-auto px-4 py-8 sm:py-12">
                    ...
                    >
                        <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${percentage >= 80 ? 'bg-gradient-to-br from-pink-400 to-pink-500' :
                            percentage >= 60 ? 'bg-gradient-to-br from-pink-400 to-pink-500' :
                        ...
                        <div className="grid grid-cols-3 gap-4 mb-8">
                            ...
                            <div className="bg-pink-50 rounded-xl p-3">
                                <div className="text-2xl font-bold text-pink-600">{grade}</div>
                                <div className="text-xs text-pink-700">Grade</div>
                            </div>
                        </div>

                        <p className="text-gray-600 text-sm mb-8">
                            {percentage >= 80 ? "Hebat! Kamu sudah paham materi ini." :
                                percentage >= 60 ? "Bagus! Kamu sudah mengerti, terus berlatih ya." :
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            <button
                                ...
                                className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 bg-pink-600 text-white rounded-xl hover:bg-pink-700 transition-colors font-medium"
                            >
                    ...
                    >
                        ...
                        <div className="space-y-3 mb-4">
                            {currentQuestion!.options.map((option, index) => {
                                ...
                                let optionStyle = 'bg-white border-gray-200 hover:border-pink-300 hover:bg-pink-50/50';
                                if (showCorrectness) {
                                    if (isCorrect) {
                        ...
                        <AnimatePresence>
                            {showExplanation && (
                                <motion.div
                                    ...
                                    className="bg-pink-50 rounded-xl p-4 border border-pink-100 mb-4"
                                >
                                    <h4 className="text-xs font-semibold text-pink-800 uppercase tracking-wide mb-2">
                                        Penjelasan
                                    </h4>
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-7 h-7 rounded-full text-xs font-medium transition-all ${currentIndex === index
                                        ? 'bg-pink-600 text-white ring-2 ring-pink-200 ring-offset-1'
                                        : status === 'correct'
